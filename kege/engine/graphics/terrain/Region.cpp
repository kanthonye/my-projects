...
namespace kege{namespace gfx{
    ...
    void Region::Submit( gfx::RenderingSystem* renderer )
    {
        if (_region.node == nullptr) return;
        
        if (_region.type == RegionType::NODE)
        {
            _region.node[0].Submit( renderer );
            _region.node[1].Submit( renderer );
            _region.node[2].Submit( renderer );
            _region.node[3].Submit( renderer );
        }
        else
        {
            if (_region.tile->IsInit())
            {
                _region.tile->Submit( renderer );
            }
    ...
    void Region::CreateTerrainTile( Terrain* terrain )
    {
        ...
        if (manager != nullptr)
        {
            manager->AddTask( new TerrainLoadTask(tx, tz, _region.tile, &terrain->GetHeightmap()) );
        }
    }
