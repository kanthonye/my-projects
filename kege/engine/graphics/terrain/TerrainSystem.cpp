...
namespace kege{namespace gfx{
    ...
    bool TerrainSystem::Init()
    {
        ...
        // temp code
        Gameobject* object;
        Gameobjects& objects = GetScene()->GetRootNode().GetChildren();
        for (Gameobjects::iterator i=objects.begin(); i!=nullptr; i++)
        {
            object = (*i);
            double h = terrain->GetHeightmapHeight( object->GetWorldPosition() );
            if (object->GetWorldPosition().y <= h)
            {
                phx::PhysicsComponent* physics = nullptr;
                if ( object->GetType() == gfx::Gameobject::CAMERA ||  object->GetType() == gfx::Gameobject::ENTITY )
                {
                    physics = (phx::PhysicsComponent*) ((gfx::Entity*) object)->GetComponent( gfx::Component::PHYSICS );
                }
                if ( physics == nullptr )
                {
                    object->GetLocalPosition() .y = h;
                }
                printf("%f\n", h);
            }
        }
