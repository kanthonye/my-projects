...
#include "../../../graphics/core/framework/LoadSHDR.hpp"
namespace kege{namespace core{
    ...
    std::pair<int,ds::string> GetShaderFile(kege::BufferReader& reader)
    {
        ...
        char name[128];
        reader.read(name, 64, "\b \n\t\r");
        if      (strcmp(name,          "VERTEX_SHADER") == 0) target = core::SHADER_VERTEX;
        else if (strcmp(name,        "FRAGMENT_SHADER") == 0) target = core::SHADER_FRAGMENT;
        else if (strcmp(name,        "GEOMETRY_SHADER") == 0) target = core::SHADER_GEOMETRY;
        else if (strcmp(name, "TESS_EVALUATION_SHADER") == 0) target = core::SHADER_TESS_EVAL;
        else if (strcmp(name,    "TESS_CONTROL_SHADER") == 0) target = core::SHADER_TESS_CTROL;
