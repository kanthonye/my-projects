...
namespace kege {namespace gfx{
    ...
    public:
        ...
        void copy(const gfx::DataBuffer<tvar>& bufr);
    ...
    template <typename tvar> tvar gfx::DataBuffer<tvar>::null_var = 0x0;
    
    typedef gfx::DataBuffer<cg::uint> IndexBuffer;
}}
...
namespace kege{ namespace gfx{
    ...
    template <typename tvar>
    void gfx::DataBuffer<tvar>::copy(const tvar* data, long stride, long count)
    {
        clear();
    ...
    template <typename tvar>
    void gfx::DataBuffer<tvar>::copy(const gfx::DataBuffer<tvar>& bufr)
    {
    ...
    template <typename tvar>
    void gfx::DataBuffer<tvar>::setdata(tvar * data, long stride, long count)
    {
        if(buffer.data != nullptr)
        {
            delete [] buffer.data;
    ...
    template <typename tvar>
    void gfx::DataBuffer<tvar>::add(tvar x)
    {
        operator +=(x);
    }
    
    template <typename tvar>
    tvar const* gfx::DataBuffer<tvar>::getdata()const
    {
        return buffer.data;
    }
    
    template <typename tvar>
    tvar * gfx::DataBuffer<tvar>::getdata()
    {
        return buffer.data;
    }
    ...
    template <typename tvar>
    void gfx::DataBuffer<tvar>::add(const tvar* data, long size)
    {
    ...
    template <typename tvar>
    void gfx::DataBuffer<tvar>::operator +=(tvar a)
    {
        if (buffer.size <= buffer.count) return;
    ...
    template <typename tvar>
    void gfx::DataBuffer<tvar>::setbuffer(long size, short stride, long count)
    {
        setsize(size);
        setcount(count);
        setstride(stride);
    ...
    template <typename tvar>
    void gfx::DataBuffer<tvar>::setcount(long size)
    {
        buffer.count = size;
    }
    
    template <typename tvar>
    void gfx::DataBuffer<tvar>::setsize(long size)
    {
        if (buffer.data != nullptr)
        {
            delete [] buffer.data;
    ...
    template <typename tvar>
    void gfx::DataBuffer<tvar>::resize(long size)
    {
        gfx::DataBuffer<tvar> vb;
        vb.copy(*this);
    ...
    template <typename tvar>
    bool gfx::DataBuffer<tvar>::empty()const
    {
        return count() == 0;
    }
    
    template <typename tvar>
    long gfx::DataBuffer<tvar>::count()const
    {
        return buffer.count;
    }
    
    template <typename tvar>
    long gfx::DataBuffer<tvar>::size()const
    {
        return buffer.size;
    }
    
    template <typename tvar>
    void gfx::DataBuffer<tvar>::clear()
    {
        if (buffer.data != nullptr)
        {
            delete [] buffer.data;
