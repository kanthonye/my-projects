...
namespace kege{namespace dae{
    ...
    void BuildMeshIndexBuffer(gfx::DataBuffer<cg::uint>& output, const gfx::DataBuffer<cg::uint>& input, short offset, short total_input)
    {
    ...
    long GetMeshIndex(short index, short offset, short total_input, const gfx::DataBuffer<cg::uint>& indices)
    {
    ...
    (   gfx::Mesh& mesh
     ,  const gfx::DataBuffer<cg::uint>&  vcount
     ,  const gfx::DataBuffer<cg::uint>&  vi, short total_input
     ,  const gfx::DataBuffer<float>& vp, short posi_offset
     ,  const gfx::DataBuffer<float>& vn, short norm_offset
     ,  const gfx::DataBuffer<float>& vt, short texr_offset
     ,  const gfx::DataBuffer<float>& vc, short colr_offset
     ,  const gfx::DataBuffer<float>& vw
     ,  const gfx::DataBuffer<float>& vj )
    {
        gfx::DataBuffer<float>& vbo = mesh.GetVertexBuffer();
        cg::vec3 min(60000.0);
        cg::vec3 max(-60000.0);
        cg::vec3 cen(0.0);
        ...
        if (!vp.empty())
        {
            mesh.GetVertexBuffer().GetVertexLayout().AddElem(gfx::VERTEX_POSITION, vp.getstride(), stride);
            stride += vp.getstride();
        }
        if (!vn.empty())
        {
            mesh.GetVertexBuffer().GetVertexLayout().AddElem(gfx::VERTEX_NORMAL, vn.getstride(), stride);
            vn_offset = stride;
            stride += vp.getstride();
        }
        if (!vt.empty())
        {
            mesh.GetVertexBuffer().GetVertexLayout().AddElem(gfx::VERTEX_TEXCOORD, vt.getstride(), stride);
            vt_offset = stride;
            stride += vt.getstride();
        }
        if (!vw.empty())
        {
            mesh.GetVertexBuffer().GetVertexLayout().AddElem(gfx::VERTEX_WEIGHT, vw.getstride(), stride);
            vw_offset = stride;
            stride += vw.getstride();
        }
        if (!vj.empty())
        {
            mesh.GetVertexBuffer().GetVertexLayout().AddElem(gfx::VERTEX_JOINT, vp.getstride(), stride);
            vj_offset = stride;
            stride += vj.getstride();
        }
        if (!vc.empty())
        {
            mesh.GetVertexBuffer().GetVertexLayout().AddElem(gfx::VERTEX_COLOR, vp.getstride(), stride);
            vc_offset = stride;
            stride += vc.getstride();
        }
    ...
    gfx::SharedMesh ColladaGeometries::InstanceGeometry(const ds::string& id)
    {
    ...
    void ColladaGeometries::ContructGeometry(gfx::Mesh& mesh, const ds::string& id, gfx::DataBuffer<float>& vw, gfx::DataBuffer<float>& vj)
    {
        ...
        gfx::DataBuffer<cg::uint> v, vcount;
        ToPolygonBuffers(vcount, v, polylist);
        
        gfx::DataBuffer<float> vp = ToVertexBuffer(vertices);
        gfx::DataBuffer<float> vn = ToVertexBuffer(normals);
        gfx::DataBuffer<float> vt = ToVertexBuffer(tcoords);
        gfx::DataBuffer<float> vc = ToVertexBuffer(colors);
        dae::ContructGeometry(mesh, vcount, v, total_input, vp, v_offset, vn, n_offset, vt, t_offset, vc, c_offset, vw, vj);
    }
    
    void ColladaGeometries::ContructGeometry(gfx::Mesh& mesh, const ds::string& id)
    {
        gfx::DataBuffer<float> vj, vw;
