...
//        quad[5] = *hmap(gx+1, gz+1);
    ...
    ,   tcoord(t)
    {
        SetRadius( sqrt( 2 * cg::sq(system->tile_size * system->scale * 0.5) ) );
        
        // GetLocal().translation is the topleft corner position of the root node
        GetLocal().translation.x  = tcoord.x * system->tile_size;
        GetLocal().translation.z  = tcoord.y * system->tile_size;
