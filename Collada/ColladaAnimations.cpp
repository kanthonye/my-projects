...
namespace kege{namespace dae{
    ...
    float ColladaAnimations::ToKeyframeChannel(gfx::KeyframeChannel* channel, kege::XML* animation)
    {
        ...
        gfx::VertexBuffer input = ToVertexBuffer(input_source);
        gfx::VertexBuffer output = ToVertexBuffer(output_source);
        
        channel->target = GetChannelTarget(animation);
        channel->keys.setsize(input.size());
        channel->keys.setcount(input.size());
