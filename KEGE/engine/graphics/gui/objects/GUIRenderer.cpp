...
namespace kege{namespace cg{
    ...
    void GUIRenderer::Init()
    {
        ...
        if (_vao != nullptr)
        {
            ...
            _vao->Bind();
            {
                ...
                _vao->AttachVBO(max_draw_count * stride * sizeof(float), stride * sizeof(float), data.data(), core::DYNAMIC_DRAW, layout);
                _draw_mode = gfx::Graphics::GetConstant(core::DRAW_TRIANGLE_STRIP);
            }
            _vao->Unbind();
        }
    ...
    void GUIRenderer::Flush()
    {
        ...
        {
            _vao->Bind();
            _vao->GetVBO()->UpdateBufferData( draw_count * stride * sizeof(float), data.data() );
            _vao->DrawArraysInstanced( _draw_mode, 0, 4, draw_count );
            _vao->Unbind();
            draw_count = 0;
        }
        zdepth = 0.0;
