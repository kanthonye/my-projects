...
namespace kege{namespace gfx{
    ...
    void ComputeTangent(core::VertexBuffer& vertex_buffer, core::VertexLayout& vertex_layout, core::IndexBuffer& indices)
    {
        // vertex position and vertex texcoord is necessary to compute vertex tangent
        const int poffset = vertex_layout.GetElem( core::VERTEX_POSITION ).offset;
        const int toffset = vertex_layout.GetElem( core::VERTEX_TEXCOORD ).offset;
        ...
        const long count = vertex_buffer.count();
        const int stride = vertex_buffer.getstride();
        vertex_buffer.resize(stride+3 * count);
        vertex_buffer.setstride(stride+3);
        const int tang_offset = stride;
        
        vertex_layout.AddElem( core::VERTEX_TANGENT, 3, tang_offset );
        const int newstride = vertex_buffer.getstride();
        if (indices.count() == 0) return;
        
        float* bufr = vertex_buffer.getdata();
    ...
    void ComputeTangent(core::VertexBuffer& vertices, core::VertexLayout& layout)
    {
        const int poffset = layout.GetElem( core::VERTEX_POSITION ).offset;
        const int toffset = layout.GetElem( core::VERTEX_TEXCOORD ).offset;
        if (poffset < 0 || toffset < 0) return;
        
        const long count    = vertices.count();
        const int minstride = vertices.getstride();
        const int maxstride = minstride+3;
        layout.AddElem( core::VERTEX_TANGENT, 3, minstride );
        vertices.resize(maxstride * count);
        vertices.setstride(maxstride);
        float* bufr = vertices.getdata();
