...
namespace kege{namespace dae{
    ...
    public:
        ...
        void GetVertexWeights(core::DataBuffer<float>& vw, core::DataBuffer<float>& vj, kege::XML* skin);
        void SetupSkeletalJointmap(gfx::Joint* hierarchy, gfx::Skeleton* skeleton);
        
        shared::Skeleton BuildSkeleton(kege::XML* instance, dae::ColladaVisualScenes& scenes);
        shared::Mesh InstanceController(const ds::string& id, dae::ColladaGeometries geometries);
        
        core::DataBuffer<float> GetInverseBindMatrices(kege::XML* controller);
        core::DataBuffer<ds::string> GetJoints(kege::XML* controller);
